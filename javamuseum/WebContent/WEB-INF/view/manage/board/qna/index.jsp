							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0" summary="관리자 관리목록입니다.">
								...
								%>
									<tr <%=topClass%>>
										...
										<td <%=targetUrl%> class="title"><%=data.getMemberid()%></td>
										<td <%=targetUrl%> class="title"><%=data.getFilename()%></td>
										<td <%=targetUrl%> class="title"><%=data.getTitle()%></td>
										<td <%=targetUrl%> class="title"><%=data.getContents()%></td>
										<td <%=targetUrl%>><%=DateUtil.getDateFormat(data.getRegistdate())%></td>
										
										<td class="last"><input type="button" value="답변" onclick="goDelete(<%=data.getNo()%>);"/></td>
									</tr>
								<%
										}
									 }
